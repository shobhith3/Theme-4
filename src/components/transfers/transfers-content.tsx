...
import { Truck, Package, Clock, CheckCircle2, ArrowRight, Loader2 } from "lucide-react";
import { dispatchTransfer, receiveTransfer } from "@/app/actions/transfer-actions";
import { useRouter } from "next/navigation";

export default function TransfersContent({ initialTransfers }: { initialTransfers: any[] }) {
  ...
  const filteredTransfers = useMemo(() => {
    switch (activeTab) {
      ...
      case "in_transit": return transfers.filter(t => t.status === "dispatched" || t.status === "in transit");
  ...
  const handleReceive = (id: string) => {
    startTransition(async () => {
      try {
        ...
        alert("Failed to receive: " + err.message);
      }
    });
  };
              ...
              <div className="flex items-center gap-3 shrink-0">
                {activeTab === "pending" && (
                  <button 
                    ...
                    className="h-[36px] px-4 bg-[var(--color-accent)] text-white rounded-md text-[13px] font-medium hover:bg-[var(--color-accent-hover)] transition-colors shadow-sm flex items-center gap-2 disabled:opacity-50"
                  >
                ...
                {activeTab === "in_transit" && (
                  <button 
                    ...
                    className="h-[36px] px-4 bg-success text-white rounded-md text-[13px] font-medium hover:bg-success/90 transition-colors shadow-sm flex items-center gap-2 disabled:opacity-50"
                  >
