...
import { CheckCircle2, Truck, Package, X, Loader2 } from "lucide-react";
import { receivePurchaseOrder } from "@/app/actions/po-actions";
import { useRouter } from "next/navigation";

export default function PurchaseOrdersContent({ initialOrders }: { initialOrders: any[] }) {
  ...
  const filteredOrders = useMemo(() => {
    ...
    return purchaseOrders.filter(po => po.status.toLowerCase() === activeTab.toLowerCase());
  }, [purchaseOrders, activeTab]);

  const handleReceive = (e: React.MouseEvent, poId: string, branchId: string) => {
    e.stopPropagation();
    startTransition(async () => {
      try {
        await receivePurchaseOrder(poId, branchId);
        alert("Purchase Order Received successfully!");
        router.refresh();
      } catch (err: any) {
        alert("Failed to receive PO: " + err.message);
      }
    });
  };
  ...
  const columns = [
    ...
    { header: "Items", cell: (item: any) => `${item.lineItems.length}`, align: "right" as const },
    { header: "Value", cell: (item: any) => `₹${item.totalAmount.toLocaleString()}`, align: "right" as const },
    { header: "Expected Delivery", cell: (item: any) => new Date(item.expectedDeliveryDate).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' }) },
    {
      header: "Status",
      cell: (item: any) => <StatusBadge status={item.status.toLowerCase() === 'sent' || item.status.toLowerCase() === 'in transit' ? 'In Transit' : item.status.toLowerCase() === 'received' || item.status.toLowerCase() === 'fulfilled' ? 'Delivered' : item.status.toLowerCase() === 'approved' || item.status.toLowerCase() === 'confirmed' ? 'Approved' : item.status.toLowerCase() === 'draft' ? 'Draft' : 'Review'} />
    },
    {
      header: "Action",
      cell: (item: any) => (
        <div className="flex justify-end">
          {(item.status.toLowerCase() === "sent" || item.status.toLowerCase() === "confirmed") ? (
            <button 
              disabled={isPending}
              onClick={(e) => handleReceive(e, item.id, item.branchId || "cm4o00a3z0000jtwck18pbbw1")}
              className="flex items-center gap-1.5 px-3 py-1.5 bg-[var(--color-accent)] text-white rounded text-[12px] font-medium hover:bg-[var(--color-accent-hover)] transition-colors disabled:opacity-50"
            >
            ...
            <span className="text-[12px] text-text-muted">
              {item.status.toLowerCase() === "fulfilled" || item.status.toLowerCase() === "received" ? "Received" : "Pending Dispatch"}
            </span>
          )}
        </div>
      ),
        ...
        <div className="fixed inset-0 z-50 flex justify-end bg-black/20 backdrop-blur-sm transition-opacity">
          <div className="w-[480px] h-full bg-white shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
            <div className="flex items-center justify-between p-5 border-b border-border/50">
              <div className="flex flex-col">
                <h3 className="text-[18px] font-bold text-text-primary flex items-center gap-2">
                  {selectedPO.poNumber}
                  <StatusBadge status={selectedPO.status.toLowerCase() === 'sent' || selectedPO.status.toLowerCase() === 'in transit' ? 'In Transit' : selectedPO.status.toLowerCase() === 'received' || selectedPO.status.toLowerCase() === 'fulfilled' ? 'Delivered' : 'Approved'} />
                </h3>
            ...
            <div className="p-6 flex flex-col gap-6 overflow-y-auto">
              ...
              {(selectedPO.status.toLowerCase() === "sent" || selectedPO.status.toLowerCase() === "confirmed") && (
                <div className="mt-4 p-4 bg-blue-50 border border-blue-100 rounded-xl flex flex-col gap-3">
                  ...
                  <button 
                    disabled={isPending}
                    onClick={(e) => {
                      handleReceive(e, selectedPO.id, selectedPO.branchId || "cm4o00a3z0000jtwck18pbbw1");
                      setSelectedPO(null);
                    }}
                    className="w-full flex items-center justify-center gap-2 py-2 bg-blue-600 text-white rounded-md text-[13px] font-bold hover:bg-blue-700 transition-colors shadow-sm disabled:opacity-50"
                  >
